...
Para crear una matriz, debe declarar una variable del tipo de matriz deseado y luego usar la palabra clave `new` para crear el objeto de matriz y asignarlo a la variable:
...
También puedes combinar la declaración y la creación de la matriz en una sola declaración:
...
- `false` para booleano
- `null` para tipos de referencia. 

Sin embargo, también puedes inicializar explícitamente una matriz durante la creación:
...
Si no necesita especificar los valores en el momento de la declaración, puede dejar algunos o todos los elementos sin inicializar:
...
Una matriz anónima es una matriz que se declara e inicializa en una sola declaración sin asignarla a una variable:
...
Las matrices anónimas se utilizan a menudo cuando se pasa una matriz como argumento a un método:
...
Sin embargo, las matrices anónimas no se limitan a los argumentos del método. Se pueden usar en cualquier lugar donde se espere una matriz, como en una tarea:
...
Para acceder a un elemento de una matriz, utilice el nombre de la matriz seguido del índice del elemento entre corchetes:
...
También puedes usar una variable para el índice:
...
Para saber la cantidad de elementos en una matriz, puede usar el atributo de longitud:
...
Si bien no puedes cambiar el tamaño de una matriz, puedes copiar el contenido de una matriz a otra:
...
Sin embargo, esto no es lo mismo que asignar una matriz a otra:
...
Para declarar una matriz multidimensional, especifique cada dimensión adicional con otro conjunto de corchetes. Por ejemplo, así es como declararías una matriz bidimensional de números enteros:
...
Luego puede crear la matriz con la palabra clave "nueva":
...
Al igual que con las matrices unidimensionales, puedes combinar la declaración y la creación:
...
También puede inicializar la matriz al momento de su creación:
...
Acceder a elementos en una matriz multidimensional es similar a una matriz unidimensional, pero ahora necesita especificar un índice para cada dimensión:
...
También puedes usar bucles anidados para iterar sobre una matriz multidimensional:
...
Tenga en cuenta también que, si bien todas las matrices de la segunda dimensión tienen la misma longitud en este ejemplo, esto no es un requisito. Puede tener una matriz *irregular* donde cada matriz en la segunda dimensión tiene una longitud diferente:
...
El método `sort()` ordena los elementos de una matriz en orden ascendente. Tiene varias sobrecargas para diferentes tipos de arreglos:
...
También puede ordenar una parte de una matriz especificando los índices inicial (inclusive) y final (exclusivo):
...
Para matrices de objetos, los objetos deben implementar la interfaz `Comparable` para que `sort()` funcione. Alternativamente, puede proporcionar un objeto `Comparador` para definir el orden de clasificación:
...
El método `binarySearch()` busca un elemento específico en una matriz ordenada utilizando el algoritmo de búsqueda binaria. Si se encuentra el elemento, devuelve su índice. Si no, devuelve un valor negativo.
...
El método `compare()` compara dos matrices lexicográficamente (por orden del diccionario). Devuelve un valor negativo si la primera matriz es *menor que* la segunda, un valor positivo si la primera matriz es *mayor que* la segunda y cero si son iguales.
...
Aquí hay un ejemplo del uso de `fill()` para llenar una matriz completa:
...
También puedes llenar solo una parte de una matriz:
...
El método `fill()` tiene sobrecargas para todos los tipos primitivos y para las referencias a objetos. Cuando se usa con referencias a objetos, cada elemento apuntará al mismo objeto:
...
Aquí hay un ejemplo con objetos inmutables (cadenas):
...
Sin embargo, cuando se usa `Arrays.fill()` con objetos mutables, cada elemento hará referencia al mismo objeto. Si modifica una instancia, todos los elementos de la matriz reflejarán ese cambio:
...
Sin embargo, el borrado de tipos impone algunas limitaciones. Por ejemplo, no puede utilizar el operador `instanceof` directamente con tipos genéricos. Si intentas algo como:
...
Otra limitación es que no puedes crear matrices de tipos parametrizados. Entonces, no puedes hacer algo como:
...
Por ejemplo, digamos que desea crear una clase genérica simple llamada "Pair", que contiene dos valores de tipos potencialmente diferentes:
...
A continuación se muestra un ejemplo de un método genérico:
...
Puedes invocar este método genérico con matrices de diferentes tipos:
...
Aquí hay otro ejemplo de un método genérico que devuelve un valor:
...
Puede invocar este método y asignar el resultado a una variable del tipo apropiado:
...
A continuación se muestra un ejemplo de cómo especificar explícitamente el tipo de argumento:
...
Y aquí hay un ejemplo de inferencia de tipos:
...
Los parámetros genéricos funcionan de manera similar con los constructores:
...
En este ejemplo, la clase `GenericBox` tiene un constructor que acepta un argumento genérico de tipo `T`. Para crear una instancia de `GenericBox` con un tipo específico, puedes pasar el argumento genérico al llamar al constructor:
...
También puedes pasar argumentos genéricos a un método de fábrica estático, por ejemplo:
...
Aquí, el método `createList` es un método de fábrica estático que crea una nueva `ArrayList` basada en los elementos proporcionados. Para pasar argumentos genéricos al llamar a este método, puede utilizar la siguiente sintaxis:
...
Es importante tener en cuenta que en muchos casos, el compilador de Java puede inferir los argumentos de tipo genérico en función del contexto, como los tipos de argumentos del método o la asignación de variables. En tales casos, puedes omitir el argumento genérico explícito y dejar que el compilador lo infiera automáticamente:
...
Sin embargo, también puedes devolver un tipo diferente según la entrada, por ejemplo:
...
O puede devolver una colección genérica:
...
Y aquí hay un ejemplo de devolución de un tipo genérico basado en múltiples parámetros de tipo:
...
Al llamar a estos métodos, el compilador determinará qué versión invocar según la cantidad y los tipos de argumentos proporcionados.
...
Repasemos un ejemplo para entender cómo implementar interfaces genéricas:
...
Así es como puedes usar las clases `StringProcessor` e `IntegerProcessor`:
...
A continuación se muestra un ejemplo de creación de un registro genérico:
...
Crear y usar instancias del registro genérico "Pair" es sencillo:
...
A continuación se muestra un ejemplo de cómo llamar al método `printList`:
...
A continuación se muestra un ejemplo de cómo llamar al método `sumNumbers`:
...
A continuación se muestra un ejemplo de cómo llamar al método `addNumbers`:
...
Al declarar una colección, podemos aprovechar el *Operador Diamante* (`<>`) para especificar el tipo:
...
Hay varias operaciones comunes que podemos realizar en una colección. Para agregar un solo elemento, usamos el método `add`:
...
Para agregar todos los elementos de otra colección, use `addAll`:
...
Eliminamos elementos con el método `remove`, especificando el objeto a eliminar o su índice para colecciones ordenadas:
...
El método `size` devuelve el número de elementos actualmente en la colección:
...
Para eliminar todos los elementos de una colección, llame al método "clear":
...
El método `contains` comprueba si una colección contiene un elemento específico y devuelve `true` si se encuentra o `false` en caso contrario:
...
El método `removeIf` permite eliminar todos los elementos que satisfacen un predicado determinado:
...
El método `forEach` (que en realidad proviene de la interfaz `java.lang.Iterable`) realiza una acción determinada en cada elemento de la colección:
...
El método `equals` comprueba si otro objeto es igual a la colección. Para que dos colecciones se consideren iguales, deben contener los mismos elementos en el mismo orden (para colecciones ordenadas) o los mismos elementos en cualquier orden (para colecciones desordenadas):
...
Con la introducción de colecciones secuenciadas en Java 21, ahora tenemos métodos consistentes para trabajar con el primer y último elemento de las colecciones con un orden de encuentro definido:
...
### Creando una `Lista`
El método más común para crear una instancia de "Lista" es utilizar un constructor:
...
También puedes crear una `Lista` a partir de una matriz usando el método `Arrays.asList`:
...
También puedes usar los métodos de fábrica `List.of` y `List.copyOf` para crear listas no modificables:
...
La interfaz `List` proporciona varios métodos para trabajar con sus elementos. El método `add` inserta un elemento en una posición especificada o lo agrega al final de la `Lista`:
...
Los métodos `get` y `set` le permiten acceder y modificar elementos según sus índices:
...
Para eliminar un elemento, utilice el método `remove`, especificando el objeto a eliminar o su índice:
...
El método `replaceAll` aplica una función dada a cada elemento de la `List`, reemplazando cada elemento con el resultado de la función:
...
Para ordenar los elementos de una "Lista", utilice el método "sort":
...
Para convertir una `Lista` en una matriz, use el método `toArray`:
...
### Creando un `Conjunto`
Puedes crear un `Set` usando un constructor, al igual que con las listas:
...
Alternativamente, también puedes usar los métodos de fábrica `Set.of` y `Set.copyOf` para crear conjuntos no modificables:
...
La interfaz `Set` proporciona varios métodos para trabajar con sus elementos. El método `add` inserta un elemento en el `Set` si aún no está presente:
...
Para verificar si un elemento está presente en el `Set`, use el método `contains`:
...
Para eliminar un elemento del `Set`, utilice el método `remove`:
...
El método `size` devuelve el número de elementos en el `Set`:
...
Para iterar sobre los elementos de un `Set`, puedes usar un bucle `for-each` o el método `forEach`:
...
### Creando un `Deque`
Puedes crear un `Deque` usando un constructor, similar a otros tipos de colección:
...
También puede especificar una capacidad inicial para un `ArrayDeque`:
...
Para una `LinkedList`, puedes crear una deque vacía o inicializarla con otra colección:
...
La interfaz Deque proporciona varios métodos para trabajar con elementos en ambos extremos del deque. Los métodos `addFirst` y `addLast` insertan elementos al principio y al final del deque, respectivamente:
...
Los métodos `getFirst` y `getLast` recuperan, pero no eliminan, los elementos al principio y al final del deque. Si el deque está vacío, arrojan una `NoSuchElementException`:
...
Para eliminar y devolver los elementos al principio y al final de la deque, utilice los métodos `removeFirst` y `removeLast`. Si el deque está vacío, arrojan una `NoSuchElementException`:
...
La interfaz `Deque` también proporciona métodos para usar deque como pila. El método `push` inserta un elemento al principio del deque, el método `pop` elimina y devuelve el elemento al principio, y el método `peek` recupera, pero no elimina, el elemento al principio:
...
- `offerFirst` y `offerLast`: insertan elementos al principio y al final del deque, respectivamente. Devuelven un valor "booleano" que indica si la inserción fue exitosa.
...
- `peekFirst` y `peekLast`: recuperan, pero no eliminan, los elementos al principio y al final del deque. Devuelven "nulo" si la deque está vacía.
...
- `pollFirst` y `pollLast`: eliminan y devuelven los elementos al principio y al final del deque. Devuelven "nulo" si la deque está vacía.
...
### Creando un `Mapa`
Puedes crear un `Mapa` usando un constructor, similar a otros tipos de colección:
...
También puedes crear un `Map` con una capacidad inicial y un factor de carga (para `HashMap` y `LinkedHashMap`):
...
- `containsKey`: Devuelve `true` si el mapa contiene la clave especificada.
...
- `containsValue`: Devuelve `true` si el mapa contiene el valor especificado.
...
- `entrySet`: Devuelve una vista `Set` de las entradas en el mapa.
...
- `forEach`: Realiza la acción dada para cada entrada en el mapa.
...
- `get`: Devuelve el valor asociado con la clave especificada, o `null` si no se encuentra la clave.
...
- `getOrDefault`: Devuelve el valor asociado con la clave especificada, o el valor predeterminado dado si no se encuentra la clave.
...
- `keySet`: Devuelve una vista `Set` de las claves en el mapa.
...
- `merge`: si la clave especificada aún no está asociada con un valor o está asociada con `null`, la asocia con el valor no nulo dado. De lo contrario, reemplaza el valor asociado con los resultados de la función de reasignación dada.
...
- `put`: Asocia el valor especificado con la clave especificada en el mapa.
...
- `putIfAbsent`: si la clave especificada aún no está asociada con un valor (o está asignada a `null`), la asocia con el valor dado y devuelve `null`; de lo contrario, devuelve el valor actual.
...
- `remove`: Elimina la entrada de la clave especificada del mapa, si está presente.
...
- `reemplazar`: reemplaza la entrada de la clave especificada solo si actualmente está asignada a algún valor.
...
- `replaceAll`: reemplaza el valor de cada entrada con el resultado de invocar la función dada en esa entrada hasta que todas las entradas hayan sido procesadas o la función arroje una excepción.
...
- `valores`: Devuelve una vista de `Colección` de los valores contenidos en el mapa.
...
A continuación se muestra un ejemplo de una clase personalizada con los métodos `hashCode` y `equals` anulados correctamente:
...
El método `Objects.hash` es un método de utilidad estático proporcionado por la clase `java.util.Objects` que genera un código hash para una secuencia de valores de entrada. Aquí está la sintaxis general del método `Objects.hash`:
...
A continuación se muestra un ejemplo de una clase `Persona` que implementa `Comparable`:
...
Al implementar el método `compareTo`, es importante manejar los valores `null` correctamente para evitar una `NullPointerException`. Puedes hacer esto agregando una verificación "nula" al comienzo del método:
...
A continuación se muestra un ejemplo de un método "equals" que es coherente con el método "compareTo":
...
A continuación se muestra un ejemplo de un "Comparador" que compara objetos "Persona" por la longitud de su nombre:
...
- `comparando`: Crea un comparador basado en una función que extrae una clave `Comparable` de un tipo.
...
- `comparingDouble`, `comparingInt`, `comparingLong`: crea comparadores basados ​​en funciones que extraen claves `double`, `int` o `long` de un tipo.
...
- `naturalOrder`, `reverseOrder`: crea comparadores basados ​​en el orden natural o en el orden inverso del orden natural de un tipo.
...
- `reversed`: Invierte el orden de un comparador.
...
- `thenComparing`, `thenComparingDouble`, `thenComparingInt`, `thenComparingLong`: Permite encadenar comparadores.
...
A continuación se muestra un ejemplo que demuestra la flexibilidad de "Comparator":
...
El método `Collections.sort` ordena una `Lista` usando su orden natural (definido por la interfaz `Comparable`) o un `Comparador` proporcionado:
...
El método `Collections.binarySearch` busca un elemento en una `Lista` ordenada utilizando el algoritmo de búsqueda binaria. La "Lista" debe ordenarse en orden ascendente según el orden natural ("Comparable") o el "Comparador" proporcionado:
...
**8. ¿Cuál es el resultado de ejecutar el siguiente programa?**
...
**9. ¿Cuál será el resultado del siguiente programa cuando se utilice el `Comparador` proporcionado?**
